...
if __name__=='__main__':
    ...
    for length in length_free_bodies:
        bond_pairs += [ [n_rigids+n_prev_res+i, n_rigids+n_prev_mol+i+1 ] for i in range(length-1)]
        n_prev_res += length
    ...
    s.bonds.N = len(bond_pairs) + len(bonds_free_rigid)
    s.bonds.typeid = [0] * ( len(bond_pairs) + len(bonds_free_rigid) )
    s.bonds.group = bond_pairs + bonds_free_rigid

    sim.state.set_snapshot(s)
    
    with gsd.hoomd.open(name='ck1d_try_start.gsd', mode='wb') as fout:
        fout.append(s)
