...
if __name__=='__main__':
    ...
    with gsd.hoomd.open(name='input_stats/ck1d-center-rescaled_multi-tau_start.gsd', mode='wb') as fout:
        fout.append(s)
    ...
    system = hoomd.init.read_gsd('input_stats/ck1d-center-rescaled_multi-tau_start.gsd')
    all_p = hoomd.group.all()
    
    rigid = hoomd.md.constrain.rigid()
    rigid.set_param('R', types=[aa_type_r[ck1d_id[i]] for i in range(ck1d_length)],
                    positions=ck1d_rel_pos)
  #  print(rigid.create_bodies(False))
    rigid.create_bodies()
     
    hoomd.dump.gsd('input_stats/ck1d-rigid-rescaled_multi-tau_start.gsd', period=1, group=all_p, truncate=True)
